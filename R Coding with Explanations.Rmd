...
```{r, include=FALSE}
# Set-up configuration for GitHub
use_git_config(user.name = "KelseaDundon", user.email = "kelsea.dundon@gmail.com")
gitcreds_set()
```
...
#### Power Density
Apply Betz limit
Assume a standard air density of 1.225 kg/m³
```{r}
p <- 1.225
Cpmax = 16/27
...
The bReeze package contains many power curves in .wtg and .pow formats. For this analysis X turbines have been selected that are commericially used in offshore wind farms. 
```{r}
pc_wtg_str <- "Nordex_N80_2.5MW	Nordex_N90_2.5MW_HS"
pc_wtg <- strsplit(pc_wtg_str,"[[:space:]]")
print(pc_wtg)
pc_pow_str <- "Bonus_82.4m_2.3MW	Bonus_MKIV_600kW	Clipper_LibertyC89_2.5MW"
pc_pow <- strsplit(pc_pow_str,"[[:space:]]")
print(pc_pow)
```
