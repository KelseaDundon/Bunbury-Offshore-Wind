...
```{r}
# Power curve 
pc_10 <- pc(Power_Curves$PC_File[10])
plot(pc_10, cp=TRUE, ct=TRUE)

# Hub Height min & max
hh10_min <- Power_Curves$HH_min_m[10]
hh10_max <- Power_Curves$HH_max_m[10]

# calculate annual energy production with minimum hub height
aep_10_hhmin <- aep(profile = pf, pc = pc_10, hub.h = hh10_min) 
capacity_10_hhmin <- aep_10_hhmin$capacity
AEP_Plot_10_hhmin <- plot(aep_10_hhmin)

# calculate annual energy production with maximum hub height
aep_10_hhmax <- aep(profile = pf, pc = pc_10, hub.h = hh10_max) 
capacity_10_hhmax <- aep_10_hhmax$capacity
AEP_Plot_10_hhmax <- plot(aep_10_hhmax)
